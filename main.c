 ...
 * serve_1_robot.c
 *
 * Created: 2021-09-09 오전 11:15:09
 * Author : CTM
 */ 
...
#define		DATA_ON					PORTD = PORTD | 0x20;		//ic4094 DATA핀을 1로 출력 | output ic4094 DATA pin as 1
#define		CLK_ON					PORTD = PORTD | 0x40;		//ic4094 CLK핀을 1로 출력 | output ic4094 CLK pin to 1
#define		STR_ON					PORTD = PORTD | 0x10;		//ic4094 STR핀을 1로 출력 | output ic4094 STR pin to 1
#define		PUSH_BUTTON_LED_ON		PORTA = PORTA | 0x01;		//LED 버튼의 LED를 1로 출력 | Output the LED of the LED button as 1.

#define		DATA_OFF				PORTD = PORTD &~ (0x20);	//ic4094 DATA핀을 0으로 출력 | output ic4094 DATA pin as 0
#define		CLK_OFF					PORTD = PORTD &~ (0x40);	//ic4094 CLK핀을 0으로 출력 | output ic4094 CLK pin as 0
#define		STR_OFF					PORTD = PORTD &~ (0x10);	//ic4094 STR핀을 0으로 출력 | output ic4094 STR pin as 0
#define		PUSH_BUTTON_LED_OFF		PORTA = PORTA &~ (0x01);	//LED 버튼의 LED를 0으로 출력 | Output the LED of the LED button as 0

volatile unsigned int fndc, fndc2, fnd_com;						//4094에 static 방식으로 연결된 FND, LED 카운터 변수 | FND statically connected to 4094, LED counter variable
...
void ic4094(int data){
	...
	for(i = 0; i < 16; i++){			//ic4094 핀이 16개 이기 때문에 0~15까지 반복 | There are 16 ic4094 pins, so repeat from 0 to 15.
		if(data&(0x8000 >> i)){			//입력된 data값과 0x8000이 i값 만큼 오른쪽으로 쉬프트 했을 때 1이 되면 | If the input data value and 0x8000 become 1 when moving to the right by the i value,
			DATA_ON;					//DATA_ON
		}
		else
		DATA_OFF;					//DATA_OFF
...
void FND_LED(){
	...
	if(fndc2 < 500){	//fndc2가 500ms보다 작을 때(0.5초씩 점멸하기 위해 사용) | When fndc2 is less than 500 ms (used to blink in 0.5 second increments)
		switch(fndc){	//fndc일 때
			...
			case 1: ic4094(num[num10] + 0x0100 + red_led_dat + blue_led_dat); break;
			//ic4094(1의 자리 데이터 + 1의 자리 FND_ON + 빨간색 LED 데이터 + 파란색 LED 데이터) | ic4094 (1 digit data + 1 digit FND_ON + red LED data + blue LED data)
			case 3: ic4094(num[num1] + 0x0080 + red_led_dat + blue_led_dat); break;
			case 5: fndc = 0; break;	//fndc = 0;
		}
	}
		...
		if(fnd_com == 0){	//fnd를 0.5초씩 점멸하지 않을 때 | When fnd does not blink for 0.5 seconds
			...
			switch(fndc){	//fndc일 때
				//ic4094(10의 자리 데이터 + 10의 자리 FND_ON + 빨간색 LED 데이터 + 파란색 LED 데이터) | ic4094 (10's digit data + 10's FND_OFF and 1's FND_OFF + red LED data + blue LED data)
				case 1: ic4094(num[num10] + 0x0180 + red_led_dat + blue_led_dat); break;
				//ic4094(1의 자리 데이터 + 1의 자리 FND_ON + 빨간색 LED 데이터 + 파란색 LED 데이터) | ic4094 (1 digit data + 10's FND_OFF and 1's FND_OFF + red LED data + blue LED data)
				case 3: ic4094(num[num1] + 0x0180 + red_led_dat + blue_led_dat); break;
				case 5: fndc = 0; break;	//fndc = 0;
			}
		}
	}
	...
	red_led_dat = 0x0800;	//빨간색 LED 데이터는 0x0800이다. | Red LED data is 0x0800.
	...
	blue_led_dat = 0x0400;	//파란색 LED 데이터는 0x0400이다. | The blue LED data is 0x0400.
	...
	if((set == 1 && set_table_finish == 1) || serving_start == 1){	//설정모드에 진입하고 테이블 설정을 마칠때 | When entering setting mode and setting the finishing table
		...
		if(serving_complete == 0 || serving_complete == 2){	//테이블에 이동중일 때 또는 테이블에서 되돌아올 때 | when moving to or returning from a table
			...
			if(UDR == '+' || serving_move_cnt == 1){		//로봇 후진 | back up a robot
				...
				PORTB = 0x05;	//(GPIO_control) 모터 후진 | back up a motor
			}
			if(UDR == '=' || serving_move_cnt == 2){		//로봇 전진 | robot forward
				...
				PORTB = 0x0a;	//(GPIO_control) 모터 전진 | motor forward
			}
			if(UDR == '<' || serving_move_cnt == 3){		//로봇 좌회전 | robot turn left
				...
				PORTB = 0x04;	//(GPIO_control) 모터 좌회전 | motor turln left
			}
			if(UDR == '>' || serving_move_cnt == 4){		//로봇 우회전 | robot turn right
				...
				PORTB = 0x01;	//(GPIO_control) 모터 우회전 | motor turn right
			}
		}
	}
}
...
{
	DDRA = 0xff; DDRB = 0xff; DDRC = 0x00; DDRD = 0xf0;	//입출력 포트 설정 | Input/Output Port Settings
	...
	while (1)
	{
		...
		seg_en1 = PINC & 0x10;		//FND 엔코더 1번은 PINC의 0x10번 핀을 &한 값이다. | FND Encoder No. 1 is the value obtained by & subtracting pin 0x10 of PINC.
		seg_en2 = PINC & 0x20;		//FND 엔코더 2번은 PINC의 0x20번 핀을 &한 값이다. | FND Encoder No.2 is the value obtained by subtracting pin 0x20 of PINC.
		right_en1 = PINC & 0x08;	//오른쪽 모터 엔코더 1번은 PINC의 0x10번 핀을 &한 값이다. | Right motor encoder No. 1 is the value of & minus pin 0x10 of PINC.
		right_en2 = PINC & 0x04;	//오른쪽 모터 엔코더 2번은 PINC의 0x20번 핀을 &한 값이다. | The right motor encoder No.2 is the value of & minus pin 0x20 of PINC.
		left_en1 = PINC & 0x02;		//왼쪽 모터 엔코더 1번은 PINC의 0x10번 핀을 &한 값이다. | The left motor encoder No. 1 is the value of & minus pin 0x10 of PINC.
		left_en2 = PINC & 0x01;		//왼쪽 모터 엔코더 1번은 PINC의 0x20번 핀을 &한 값이다. | The left motor encoder No. 1 is the value of & minus pin 0x20 of PINC.
	}
}